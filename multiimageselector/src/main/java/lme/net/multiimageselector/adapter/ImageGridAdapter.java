package lme.net.multiimageselector.adapter;
        ...
        void bindData(final Image data,final int position) {
            ...
            if (showSelectIndicator) {
                ...
                if (mSelectedImages.contains(data)) {
                    // 设置选中状态
                    indicator.setImageResource(R.drawable.v1_switch_on);
                    mask.setVisibility(View.VISIBLE);
                } else {
                    // 未选择
                    indicator.setImageResource(R.drawable.v1_switch_off);
                    mask.setVisibility(View.GONE);
                }
                indicator.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        imageSelectCallback.select(ImageGridAdapter.this,position);
                    }
                });
                image.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        imageSelectCallback.edit(ImageGridAdapter.this, position);
                    }
                });
            } else {
                image.setOnClickListener(new View.OnClickListener() {
            ...
            if (mItemSize > 0) {
                // 显示图片
                ImageLoadUtil.loadPhoto(context,data.path,image);
            }
        }
    }
    public interface ImageSelectCallback{
        void select(ImageGridAdapter imageGridAdapter, int position);
        void edit(ImageGridAdapter imageGridAdapter, int position);
    }
}
